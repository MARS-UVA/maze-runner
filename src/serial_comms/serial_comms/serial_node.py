...
class SerialNode(Node):
    ...
    def readFromNucleo(self):
        ...
        if data:
            sensor_values = Feedback(front_sensor = data[0][0], right_sensor = data[0][1], left_sensor = data[0][2])
            self.get_logger().warn(f"Ultrasonic data: Front: {data[0][0]}, Right: {data[0][1]}, Left: {data[0][2]}")
